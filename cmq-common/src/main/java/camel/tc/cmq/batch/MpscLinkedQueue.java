...
/**
 * A lock-free concurrent single-consumer multi-producer {@link java.util.Queue}.
 * It allows multiple producer threads to perform the following operations simultaneously:
 * <ul>
 * <li>{@link #offer(Object)}, {@link #add(Object)}, and {@link #addAll(java.util.Collection)}</li>
 * <li>All other read-only operations:
 * <ul>
 * <li>{@link #contains(Object)} and {@link #containsAll(java.util.Collection)}</li>
 * <li>{@link #element()}, {@link #peek()}</li>
 * <li>{@link #size()} and {@link #isEmpty()}</li>
 * <li>{@link #iterator()} (except {@link java.util.Iterator#remove()}</li>
 ...
 * <li>{@link #remove(Object)}, {@link #removeAll(java.util.Collection)}, and {@link #retainAll(java.util.Collection)}</li>
...
final class MpscLinkedQueue<E> extends MpscLinkedQueueTailRef<E> implements Queue<E> {
    ...
    public MpscLinkedQueue(int size) {
        this.size = size;
    ...
    public boolean offer(E value) {
        ...
        if (count.get() > size) return false;
    ...
    public int drainTo(Collection<E> items, int size) {
        int index = 0;
        while (index < size) {
            E item = poll();
            if (item == null) break;
